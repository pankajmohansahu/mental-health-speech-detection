...
## **10.1 RAVDESS: Temporal Sequence Modeling**

The 338 file-level features do not encode how vocal characteristics evolve across the recording duration. Emotions unfold temporally — the onset, peak, and offset of an emotional expression have distinct spectral profiles. Frame-level feature sequences (e.g., MFCC extracted every 10 ms → ~300 time steps per 3 s clip) fed to a BiLSTM or Transformer encoder would capture these dynamics. Whisper's frozen encoder, already available in the project infrastructure, can directly extract contextualized frame-level embeddings from RAVDESS clips at zero additional training cost, making this the most actionable near-term improvement.

## **10.2 DAIC-WOZ: Temporal Session Modeling**

The acoustic branch currently compresses each 30-minute session into a single 323-dimensional vector via mean/std aggregation. Temporal dynamics — how vocal markers evolve during the interview — are lost. Segment-level modeling (e.g., 10-second window features fed as a sequence to an RNN) would capture fluctuations in engagement, fatigue, and emotional state over the session. This requires the raw session CSVs, which are already available in the project.

## **10.3 DAIC-WOZ: Cross-Dataset Pretraining**

The core problem is N=30 positive training examples. One concrete mitigation is pretraining a clinical speech encoder on MODMA (MDD audio, N=52) and RAVDESS (negative-affect audio) before fine-tuning on DAIC-WOZ. This would encode depression-relevant vocal biomarkers into the encoder's representations prior to the tiny fine-tuning step, reducing the effective degrees of freedom that must be estimated from DAIC-WOZ alone.

## **10.4 MODMA: Multi-Segment Ensemble**

Currently each subject is represented as a single aggregated feature vector. MODMA's raw audio files contain multiple recording sessions per subject. Training on individual segments and using majority-vote over a subject's segments at inference time would provide (a) more training examples per gradient step, and (b) a more robust prediction by averaging over session-to-session variability.

## **10.5 Threshold Calibration for Clinical Deployment**

The current binary threshold for DAIC-WOZ (optimized on val F1) is not calibrated for clinical use. A recall-prioritized threshold (F2-score optimization on the validation precision-recall curve) should be adopted for any deployment scenario, since missing a depressed patient is clinically more costly than a false alarm. A concrete analysis of the precision-recall tradeoff at different thresholds — with clinical risk quantification for each operating point — would make the model's behavior interpretable to clinicians.

## **10.6 SHAP-Guided Feature Engineering**

The SHAP analysis of the DAIC-WOZ acoustic XGBoost branch (generated in Notebook 7) identifies the top 15 features by mean absolute SHAP value. These features should be used to guide targeted feature engineering in the next milestone: computing additional statistics (percentiles, inter-quartile range, autocorrelation lag-1) specifically for the top-ranked COVAREP dimensions rather than applying uniform aggregation across all 297 COVAREP features. This is a principled way to increase the feature set's discriminative capacity without the noise penalty of undirected feature expansion.


---

# **11. Pipeline 2 Implementation and Experiment Work**

This section describes **Pipeline 2** as implemented in Milestone 4.

## **11.1 Pipeline 2 Definition**
...
## **11.2 HPT-Centered Experiment Design**
...
## **11.3 HPT Methodology Implemented in Pipeline 2**
...
## **11.4 Why HPT Was the Major Lever**
...
## **11.5 How to Report HPT Outcomes**
